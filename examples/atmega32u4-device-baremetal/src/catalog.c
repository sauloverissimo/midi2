/* Deterministic UMP catalog for the atmega32u4-device-baremetal recipe.
 *
 ...
 *
 * Pure C99, host-unit-testable (test/test_catalog.c); the source of truth
 * for the recipe's `## Spec coverage` table. */
#include "catalog.h"
#include "midi2_msg.h"
...
uint8_t midi2_catalog_build(uint32_t idx, catalog_msg_t *out) {
  ...
  switch (idx) {
     ...
     * Endpoint / Function Block / Stream Config discovery is owned by this
     * recipe's stream_responder.c, not this catalog. */
